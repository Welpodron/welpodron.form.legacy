...
if ($arActions['op'] === 'delete' && $arActions['id']) {
    RequestTable::delete($arActions['id']);
}

$gridId = RequestTable::getTableName();
$filterId = $gridId . '_filter';
$toolbarId = $gridId . '_toolbar';

$primaryKey = RequestTable::getEntity()->getPrimary();
...
foreach (RequestTable::getEntity()->getFields() as $key => $value) {
    ...
    $list[] = [
        ...
        'actions' => [
            [
                'text' => 'Удалить',
                'default' => true,
                'onclick' => 'if(confirm("Вы точно хотите удалить заявку")){document.location.href="?op=delete&id=' . $row['ID'] . '"}'
            ]
        ]
    ];
}
